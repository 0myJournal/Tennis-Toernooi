...
//post a form to upload XML file
function postFormXML($db, $splrcnt){
    ?>
    <form action="" method="post" enctype="multipart/form-data">
            <br>
            <input type="file" name="xml" class="text-input">
            <br>
            <br>
            <input type="submit" value="Registreer spelers" name="xmlsubmit" class="submit-btn">
        </form>
        <br>
        <?php
       ...
       if(isset($_POST["xmlsubmit"])){
            ...
            if(isset($_FILES['xml']))
            {
                 //check if file uploaded has no errors
                    if (($_FILES['xml']['error'] == UPLOAD_ERR_OK)) {

                        //get the file path to check the extention
                        $filename = $_FILES['xml']['name'];
                        //get the extention of path
                        $fileType = strtolower(pathinfo($filename,PATHINFO_EXTENSION));
                        //check if extention is an xml file
                        if($fileType=="xml"){
                            //loads the XML file so that I can extract players from file
                            $xml = simplexml_load_file($_FILES['xml']['tmp_name']); 
                            uploadPlayers($xml, $db, $splrcnt);
                        }
                        else{
                            echo "<h2>Bestand is geen XML bestand</h2>";
                        }
                    }
                    else{
                        echo "<h2>Er is een fout opgelopen</h2>";
                    }
            }
...
//uploads players from XML file into spelers table
function uploadPlayers($xml, $db, $splrcntindb)
{
    //check if the file does not contain too many players!
    $spelercnt=0;
    foreach($xml->aanmelding as $speler)
    {
        $spelercnt++;
    }
    //check if there aren't too many players
    $aantal = $splrcntindb+$spelercnt;
    if($aantal>128){
        $toomuch = $aantal - 128;
        echo "<h2> XML bestand bevat te veel spelers, ".$toomuch." spelers te veel</h2>";
    }
    else{
        $succesfulinserts=0;
    //get each aanmelding from XML file and 
        foreach($xml->aanmelding as $speler)
        {
            //get all the names from object aanmelding
            $voornaam = $speler->spelervoornaam;
            $achternaam = $speler->spelerachternaam;
            $tus = $speler->spelertussenvoegsels;
            $school = $speler->schoolnaam;
            //get ID from school and if it does not exist upload school to database
            $school_id = getIdFromSchoolName($school,$db);
            if($school_id=="no school"){
                insertSchoolIntoDb($school,$db);
                $school_id = getIdFromSchoolName($school,$db);
            }
            //check if the name is valid
            if(checkValidityName($voornaam,$tus,$achternaam,$school_id,$db,1))
            {
                //insert speler into database
                ...
                //check if query went well and count the succesful inserts 
                if($result){
                    $succesfulinserts++;
                }
            }
        }
        //check if all the players were inserted into the database
        if($succesfulinserts==$spelercnt){
            echo "<h2> Alle spelers zijn geregistreerd </h2>";
        }
    }
}
